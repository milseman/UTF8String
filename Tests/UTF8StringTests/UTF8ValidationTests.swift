...
public class UTF8ValidationTest: XCTestCase {
    ...
    func run(_ bytes: UnsafeBufferPointer<UInt8>) throws {
        if case .error(let range) = validateUTF8(bytes) {
    ...
    func assertInvalidUTF8(_ bytes: UnsafeBufferPointer<UInt8>,
        ...
        do {
            ...
            let repaired = repairUTF8(bytes, firstKnownBrokenRange: expectedErrorRange)
    ...
    private let replacementChar: UTF8String.String = Unicode.Scalar._replacementCharacter.withUTF8CodeUnits {
        ...
        for illegalSequence in illegalSequences {
            for illegalStarterIndex in [0, 1, 3, 6, 10] {
                ...
                Unicode.Scalar._replacementCharacter.withUTF8CodeUnits {
                    expectedRepairedBytes.insert(contentsOf: $0, at: illegalStarterIndex)
                }
                self.assertValidUTF8(expectedRepairedBytes)
    ...
    func testInvalid_longSequenceOfTruncatedBytes() {
        ...
        let expectedOutput = UTF8String.String(decoding: (0..<1000).flatMap { _ in
            Unicode.Scalar._replacementCharacter.withUTF8CodeUnits { Array($0) }
        }, as: UTF8.self)
        longSequence.withUnsafeBufferPointer { ptr in
            ...
            let string = repairUTF8(ptr, firstKnownBrokenRange: firstBrokenSequence)
    ...
    func testInvalid_flagLeftOfSomethingBroken() {
    ...
    func testInvalid_somethingBrokenSandwichedInFlags() {
    ...
    func testInvalid_lotsOfGoodBrokenInterspersedThenMassiveGood() {
        let brokenByte = "ذ".utf8.first!
        let goodByte = "_".utf8.first!

        let brokenBytes: [UInt8] = (0..<500).flatMap {
            _ in [brokenByte, goodByte]
        } + Array(repeating: "x".utf8.first!, count: 1000)
        let expectedRepairedBytes: [UInt8] = brokenBytes.flatMap { byte -> [UInt8] in
            if byte == brokenByte {
                return Array(self.replacementChar.utf8)
            } else {
                return [byte]
            }
        }
        ...
        self.assertInvalidUTF8(brokenBytes,
                               expectedErrorRange: 0..<1,
                               expectedRepairedString: String(decoding: expectedRepairedBytes, as: UTF8.self))
    }
}
